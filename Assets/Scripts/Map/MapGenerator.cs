...
namespace Map
{
    public static class MapGenerator
    {
        ...
        {NodeType.Mystery, NodeType.Store, NodeType.AlchemyLab, NodeType.MinorEnemy, NodeType.RestSite};
        ...
        private static void PlaceLayer(int layerIndex)
        {
            ...
            for (var i = 0; i < config.GridWidth; i++)
            {
                ...
                {
                    node.position = new Vector2(0, GetDistanceToLayer(layerIndex));
                }
                else
                {
                    node.position = new Vector2((-offset/1.5f) +i * layer.nodesApartDistance, GetDistanceToLayer(layerIndex));
                }
